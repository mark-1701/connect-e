...
d.addEventListener('DOMContentLoaded', (e: Event) => {
  // sidebar
  const $menuBtn = d.querySelector('#menu-btn') as HTMLElement;
  const $sideBar = d.querySelector('.sidebar') as HTMLElement;
  // create
  const $addProductBtn = d.querySelector('#add-product-btn') as HTMLElement;
  const $createProductModal = d.querySelector('#create-product-modal') as HTMLElement;
  const $closeCreateModalBtn = d.querySelector('#close-create-modal-btn') as HTMLElement;
  const $createProductForm = d.querySelector('#create-product-form') as HTMLElement;
  // update
  const $updateProductModal = d.querySelector('#update-product-modal') as HTMLElement;
  const $closeUpdateProductModal = d.querySelector('#close-update-modal-btn') as HTMLElement;
  const $updateProductForm = d.querySelector('#update-product-form') as HTMLFormElement;

  const $table = d.querySelector('.table') as HTMLElement;
  ...
  d.addEventListener('click', (e: Event) => {
    ...
    if (target.matches('.delete-btn') || target.matches('.delete-btn *')) {
      if (confirm('¿Esta seguro de eliminar el producto?')) {
        ...
        const productData = JSON.parse(localStorage.getItem('productData') || '[]');
        const updatedProductData = productData.filter((product: Product) => product.id !== selectedProductId);
  ...
  d.addEventListener('submit', (e: Event) => {
    ...
    if (target === $createProductForm) {
      const targetForm = target as HTMLFormElement;
      e.preventDefault();
    ...
    if (e.target === $updateProductForm) {
      ...
      const updatedProductData = productData.map((product: any) =>
